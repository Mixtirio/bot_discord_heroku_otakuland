...
bot.on("message", message => {
    ...
    if(message.member.hasPermission('ADMINISTRATOR')){
        if(message.content.startsWith('!team')){
            var embed = new Discord.MessageEmbed()
            .setColor("#03075e")
            .setTitle('**Team événement**')
            .setDescription('🍜 : Kage\n🏴 : Pirate\n🧿 : Sayen\n🥋 : Titans\n🍂 : Hunter\n👺 : Pourfendeur de Démon\n🔪 : Assassins\n😈 : Ghoul')
            .addField("Veuillez cliquer sur la réaction du rôle demandé.", "🏮・OtakuLand's")
...
bot.on('message', message => {
    if(message.content.startsWith('point')){
        message.channel.send('Quelle est la team dont vous voudriez voir les points ?\n\n**!Pirates\n!Kage**')
    }
    if(message.content.startsWith('!PtsKage')){
...
bot.on("message", async message => {
    ...
    if(message.member.hasPermission('ADMINISTRATOR')){
        if(message.content.startsWith('!tirage')){
            const tirage = message.guild.members
            message.channel.send("Le gagnant est : " + tirage[Math.floor(Math.random()*tirage.length)])
        }
    }
    if(message.member.hasPermission('ADMINISTRATOR')){
        if(message.content.startsWith('!histoirerp')){
            var embed = new Discord.MessageEmbed()
            .setColor('#329da8')
            .setTitle('**Histoire RP**')
            .setDescription('Des années dans le passé, dans le village de Yokuri, le Ringin se fit assassiné par une mystérieuse personne.\n\nDes témoins on reconnu l\' amblème du village de Chiroke leurs rivaux.\n\nUne guerre éclata entre s\'est deux pays,\n jusqu\'au jour oû le clan Biwamé de Yokuri se réunissent pour mettre fin à cette guerre.\n\nMais à cause de cette guerre les Rioki (démon) de Chiroke se réveillent.\n\nQuarant ans s\'écoulères dans la peur d\'une attaque des Chirokes, mais le village des Yokuri découvrit énormément de force des les Biwamé qui deviennent les Pilliers & Guardes du village. Ils héritent leurs pouvoirs de générations en génrations\n\nJusqu\'au jours oû le nouveau Ringin de Yokuri prépara une attaque supprise sur Chiroke dans la prochaine décéni pour étendre son territoire.')
...
bot.on('guildMemberAdd', member => {
    member.guild.channels.cache.get('861262687861080094').send(`${member}`, new Discord.MessageEmbed()
        .setTitle(`Bienvenue !`)
        .setDescription(`${member} a rejoint 🏮・OtakuLand's, nous somme désormais ${member.guild.memberCount}. Bon séjour !`)
        .setColor('#00ff00')
        .setImage('https://imgur.com/6XTBMvd.jpg'))
})
...
bot.on("message", message => {
    ...
    guild.channels.cache.get('864439434781786112').send(new Discord.MessageEmbed()
    .setTitle(`Nouvelle ajout du bot !`)
    .setDescription(`Le bot à été ajouté sur ${guild.name}`)
    .setColor('#00ff00')
    .setImage('https://imgur.com/j0qLqp3'))
